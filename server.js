...
//Here we update a comment
function updateComment (url, request) {
// We need the id of the comment
// We need the savedComment
// We need the requestComment to update
// We need a response
const id = Number(url.split('/').filter(segment => segment)[1]);
...
//Here we create an if statment that checks
//if the id is not true or the requestComment is not true and
//if they aren't we display a 400 error.
//if savedComment is not trues we display a 404
//error. If not we replace the savedComment with the new requestCommment
...
//Here we delete a comment.
//We need to id of the comment
//We need the saved comment
//We need a responses
...
//Comments have comments, commentIds and articleIds associated
//with them
if (savedComment) {
// Here we create a new variable comment that takes the comment
//id to find the comment
  const comment = database.comments[id];
  // Here we set that comment at that id to null
